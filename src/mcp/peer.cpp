            ...
            if (header_end != std::string::npos) {
                ...
                for (size_t i = 0; i < header_end; i++)
                    lower.push_back(static_cast<char>(std::tolower(
                        static_cast<unsigned char>(request[i]))));
                auto pos = lower.find("content-length:");
                if (pos != std::string::npos) {
                    try { content_length = std::stoul(lower.substr(pos + 15)); }
...
static void handle_peer_client(int client_fd, ServerContext& ctx) {
    std::string request;
    if (!read_http_request(client_fd, request)) { close(client_fd); return; }
    ...
    if (method == "GET" && path == "/rpc/health") {
        send_http_response(client_fd, 200, json{
            {"pid", self_pid()},
...
static std::optional<std::string> http_post_local(int port, const std::string& path,
                                                  ...
                                                  std::string& error) {
    int fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd < 0) { error = "socket() failed"; return std::nullopt; }
...
std::optional<json> proxy_tool_call(ServerContext& ctx, const std::string& name,
    ...
    if (!pid_alive(entry->owner_pid)) {
        error = "registered owner (pid " + std::to_string(entry->owner_pid) +
                ") is no longer running";
        return std::nullopt;
    }

    json payload = {{"name", name}, {"arguments", args}};
    auto body = http_post_local(entry->owner_port, "/rpc/tool",
                                entry->owner_token, payload.dump(), error);
