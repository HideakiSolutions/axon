...
int64_t thread_create(Database& db, const std::string& name, const std::string& kind) {
    auto res = db.conn().Query(
        "SELECT id FROM threads WHERE name = '" + sq(name) + "'");
    if (!res->HasError() && res->RowCount() > 0)
        return res->GetValue<int64_t>(0, 0);

    auto ins = db.conn().Query(
        "INSERT INTO threads (id, name, kind) VALUES ("
        "nextval('seq_id'), '" + sq(name) + "', '" + sq(kind) + "') RETURNING id");
...
std::vector<Thread> thread_list(Database& db) {
    auto res = db.conn().Query(
        "SELECT t.id, t.name, t.kind, CAST(t.created_at AS VARCHAR),"
...
int64_t session_start(Database& db, int64_t thread_id, const std::string& label) {
    auto ins = db.conn().Query(
        "INSERT INTO sessions (id, thread_id, label) VALUES ("
        "nextval('seq_id'), " + std::to_string(thread_id) +
        ", '" + sq(label) + "') RETURNING id");
...
static std::string build_digest(Database& db, int64_t session_id) {
    // Fetch session info
    auto si = db.conn().Query(
        "SELECT label, CAST(started_at AS VARCHAR), CAST(ended_at AS VARCHAR)"
        " FROM sessions WHERE id = " + std::to_string(session_id));
    std::string label      = (!si->HasError() && si->RowCount() > 0) ? si->GetValue(0, 0).ToString() : "";
    std::string started_at = (!si->HasError() && si->RowCount() > 0) ? si->GetValue(1, 0).ToString() : "";
    std::string ended_at   = (!si->HasError() && si->RowCount() > 0) ? si->GetValue(2, 0).ToString() : "";

    // Collect anchored file/symbol names
    auto an = db.conn().Query(
        "SELECT DISTINCT COALESCE(f.path, '') AS fp, COALESCE(sym.name, '') AS sn"
        ...
        " WHERE t.session_id = " + std::to_string(session_id) +
        " LIMIT 20");
    ...
    auto tr = db.conn().Query(
        "SELECT t.id, t.role, t.content"
        " FROM turns t"
        " WHERE t.session_id = " + std::to_string(session_id) +
        " ORDER BY t.ts ASC");
    ...
    auto aid = db.conn().Query(
        "SELECT DISTINCT ta.turn_id FROM turn_anchors ta"
        " JOIN turns t ON t.id = ta.turn_id"
        " WHERE t.session_id = " + std::to_string(session_id));
    ...
    add_turn(0); added.insert(first_id);
...
void session_end(Database& db, int64_t session_id, EmbeddingModel* model, bool compute_digest) {
    db.conn().Query(
        "UPDATE sessions SET ended_at = now() WHERE id = " + std::to_string(session_id));
...
std::vector<Turn> session_get(Database& db, int64_t session_id, int limit) {
    auto res = db.conn().Query(
        ...
        " FROM turns WHERE session_id = " + std::to_string(session_id) +
        " ORDER BY ts ASC LIMIT " + std::to_string(limit));
...
std::vector<Session> thread_get_sessions(Database& db, int64_t thread_id) {
    auto res = db.conn().Query(
        "SELECT id, thread_id, COALESCE(label,''), CAST(started_at AS VARCHAR),"
        "       COALESCE(CAST(ended_at AS VARCHAR),''), COALESCE(digest,'')"
        " FROM sessions WHERE thread_id = " + std::to_string(thread_id) +
...
int64_t turn_add(Database& db, EmbeddingModel* model,
                 int64_t session_id, const std::string& role,
    ...
    if (model) {
        ...
        auto ins = db.conn().Query(
            "INSERT INTO turns (id, session_id, role, content, embedding) VALUES ("
            "nextval('seq_id'), " + std::to_string(session_id) +
            ", '" + sq(role) + "', '" + sq(content) + "', " +
        ...
        auto ins = db.conn().Query(
            ...
            "(nextval('seq_id'), " + std::to_string(session_id) +
            ", '" + sq(role) + "', '" + sq(content) + "') RETURNING id");
        if (ins->HasError()) throw std::runtime_error("turn_add: " + ins->GetError());
        tid = ins->GetValue<int64_t>(0, 0);
    }
...
void auto_anchor(Database& db, int64_t turn_id, const std::string& content) {
    ...
    for (auto it = file_begin; it != file_end; ++it) {
        ...
        auto res = db.conn().Query(
            "SELECT id FROM files WHERE path LIKE '%" + sq(basename) + "' LIMIT 1");
        if (!res->HasError() && res->RowCount() > 0) {
            ...
            db.conn().Query(
                "INSERT INTO turn_anchors (id, turn_id, file_id, kind) VALUES ("
                "nextval('seq_id'), " + std::to_string(turn_id) +
                ", " + std::to_string(fid) + ", 'mentions')");
        }
    }

    // ── Pass 2: detect symbol names using word-boundary scan ─────────────────
    // Load top-500 symbol names from graph (most-referenced first)
    auto sym_res = db.conn().Query(
    ...
    for (duckdb::idx_t i = 0; i < sym_res->RowCount(); i++) {
        ...
        auto sr = db.conn().Query(
            "SELECT id FROM symbols WHERE name = '" + sq(sym_name) + "' LIMIT 1");
        if (!sr->HasError() && sr->RowCount() > 0) {
            ...
            db.conn().Query(
                "INSERT INTO turn_anchors (id, turn_id, symbol_id, kind) VALUES ("
                "nextval('seq_id'), " + std::to_string(turn_id) +
                ", " + std::to_string(sid) + ", 'mentions')");
        }
    }
}
...
int64_t anchor_link(Database& db, int64_t turn_id, int64_t file_id, int64_t symbol_id,
    ...
    auto ins = db.conn().Query(
        ...
        "nextval('seq_id'), " + std::to_string(turn_id) +
        ", " + (file_id   > 0 ? std::to_string(file_id)   : "NULL") +
        ", " + (symbol_id > 0 ? std::to_string(symbol_id) : "NULL") +
        ", '" + sq(kind) + "') RETURNING id");
...
std::vector<Anchor> turn_get_anchors(Database& db, int64_t turn_id) {
    auto res = db.conn().Query(
        ...
        "  kind"
        " FROM turn_anchors WHERE turn_id = " + std::to_string(turn_id));
...
std::vector<TurnHit> turn_search(Database& db, EmbeddingModel& model,
                                 const std::string& query, int limit,
                                 int64_t thread_id) {
    auto qvec = model.embed(query);
    std::string vec_str = vec_literal(qvec, model.dims());

    std::string where = thread_id >= 0
        ? " AND s.thread_id = " + std::to_string(thread_id)
        : "";

    std::string sql =
        "SELECT t.id, t.session_id, t.role, t.content, CAST(t.ts AS VARCHAR),"
        "       array_cosine_similarity(t.embedding, " + vec_str + ") AS score,"
...
std::vector<TurnHit> turns_for_files(Database& db, EmbeddingModel& model,
    ...
    std::string sql =
        ...
        "            THEN array_cosine_similarity(t.embedding, " + vec_str + ")"
...
int embed_pending_turns(Database& db, EmbeddingModel& model, int limit) {
    ...
    for (size_t i = 0; i < ids.size(); i++) {
        ...
        std::string sql =
            "UPDATE turns SET embedding = " + vec_literal(emb, dims) +
...
std::unordered_set<std::string> load_symbol_name_cache(Database& db, int limit) {
    auto res = db.conn().Query(
        ...
        " GROUP BY s.name ORDER BY COUNT(e.id) DESC LIMIT " + std::to_string(limit));
