...
IndexStats index_project(const Config& cfg, Database& db, ProgressCallback on_progress = nullptr,
                         bool force = false);
...
IndexStats index_files(const Config& cfg, Database& db,
                       const std::vector<std::filesystem::path>& paths, bool prune = false,
...
IndexStats sync_project(const Config& cfg, Database& db, ProgressCallback on_progress = nullptr);
