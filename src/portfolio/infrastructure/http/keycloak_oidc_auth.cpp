...
[[noreturn]] void denied(const char* message) { throw std::runtime_error(message); }
...
std::vector<unsigned char> base64url_decode(const std::string& input) {
    ...
    for (char& ch : b64) {
        ...
        else if (!std::isalnum(static_cast<unsigned char>(ch))) denied("JWT base64url is invalid");
    }
    while (b64.size() % 4 != 0) b64.push_back('=');
    std::vector<unsigned char> result((b64.size() / 4) * 3);
    const auto decoded = EVP_DecodeBlock(result.data(),
...
json decode_json(const std::string& segment) {
    ...
    catch (const json::exception&) { denied("JWT JSON is invalid"); }
}
...
bool key_permits_rs256_verification(const json& jwk) {
    if (!jwk.is_object() || (jwk.contains("alg") &&
                             (!jwk.at("alg").is_string() || jwk.at("alg").get<std::string>() != "RS256")))
        return false;
...
void verify_rs256(const std::string& signing_input, const std::vector<unsigned char>& signature,
    ...
    std::unique_ptr<BIGNUM, decltype(&BN_free)> n(BN_bin2bn(modulus.data(), modulus.size(), nullptr), BN_free);
    std::unique_ptr<BIGNUM, decltype(&BN_free)> e(BN_bin2bn(exponent.data(), exponent.size(), nullptr), BN_free);
...
KeycloakOidcAuthenticator::KeycloakOidcAuthenticator(KeycloakOidcConfig config)
    ...
    if (config_.issuer.rfind("https://", 0) != 0 || config_.issuer.size() > 512 ||
        config_.audience.empty() || config_.audience.size() > 256 || config_.jwks_json.size() > 1024 * 1024)
    ...
    } catch (const json::exception&) { throw std::invalid_argument("Keycloak JWKS is invalid"); }
}

AuthenticatedPrincipal KeycloakOidcAuthenticator::authenticate_bearer(
    const std::string& authorization, std::chrono::system_clock::time_point now) const {
    ...
    if (one == std::string::npos || two == std::string::npos || token.find('.', two + 1) != std::string::npos)
    ...
    if (!header.is_object() || header.contains("crit") || header.value("alg", "") != "RS256" || !header.contains("kid") ||
        !header.at("kid").is_string() || !claims.is_object() || claims.value("iss", "") != config_.issuer ||
        !claims.contains("aud") || !audience_matches(claims.at("aud"), config_.audience) ||
        !claims.contains("sub") || !claims.at("sub").is_string() || claims.at("sub").get<std::string>().empty() ||
        !claims.contains("exp") || !claims.at("exp").is_number_integer())
        denied("JWT claims are invalid");
    const auto expires = std::chrono::system_clock::time_point{std::chrono::seconds{claims.at("exp").get<long long>()}};
    ...
    const auto key = std::find_if(jwks.at("keys").begin(), jwks.at("keys").end(), [&](const json& candidate) {
        return candidate.is_object() && candidate.value("kid", "") == kid &&
               candidate.value("use", "sig") == "sig" && key_permits_rs256_verification(candidate);
    });
    ...
    return AuthenticatedPrincipal(config_.issuer, claims.at("sub").get<std::string>(), config_.audience);
}
