...
struct Transaction {
    ...
    ~Transaction() { if (!committed) connection.Query("ROLLBACK"); }
    void commit() {
...
DuckdbPortfolioStore::DuckdbPortfolioStore()
    : database_(std::make_shared<duckdb::DuckDB>(nullptr)),
      connection_(std::make_unique<duckdb::Connection>(*database_)) { migrate(); }
...
void DuckdbPortfolioStore::migrate() {
    ...
    ok(connection_->Query("CREATE TABLE IF NOT EXISTS portfolio_schema_migrations("
                          "version INTEGER PRIMARY KEY, checksum VARCHAR NOT NULL)"), "schema ledger");
    auto migration = connection_->Query(
        "SELECT checksum FROM portfolio_schema_migrations WHERE version=1");
    ...
    ok(connection_->Query("CREATE TABLE IF NOT EXISTS portfolio_streams("
    ...
    ok(connection_->Query("CREATE TABLE IF NOT EXISTS portfolio_entities("
                          ...
                          "PRIMARY KEY(repository_id,index_stream_id,entity_kind,entity_key))"), "entity schema");
    ...
    ok(connection_->Query("CREATE TABLE IF NOT EXISTS portfolio_reidentifications("
                          ...
                          "UNIQUE(index_stream_id,sequence))"), "reidentification schema");
}
...
ProviderCapabilities DuckdbPortfolioStore::capabilities() const {
    return {ProviderRole::PortfolioStore,
            ...
            kMaxEventsPerBatch, kMaxEventsPerBatch, kMaxMutationsPerEvent,
...
ProviderHealth DuckdbPortfolioStore::health() const {
    ...
    catch (const std::exception& error) { return {ProviderHealthStatus::Unavailable, error.what()}; }
}
std::string DuckdbPortfolioStore::schema_version() const { return "axon/portfolio-store/v1"; }
std::string DuckdbPortfolioStore::protocol_version() const { return "axon/portfolio-provider/v1"; }

CursorEpochManifest DuckdbPortfolioStore::state_unlocked(const RepositoryStreamKey& stream) const {
    auto statement = connection_->Prepare("SELECT cursor,epoch,manifest,stale,removed FROM portfolio_streams "
    ...
    return {true, rows->GetValue(0, 0).GetValue<std::uint64_t>(),
            rows->GetValue(1, 0).ToString(), rows->GetValue(2, 0).ToString(),
            rows->GetValue(3, 0).GetValue<bool>(), rows->GetValue(4, 0).GetValue<bool>()};
}
...
DuckdbPortfolioStore::physical_state_unlocked(const std::string& index_stream_id) const {
    ...
    return {{rows->GetValue(0, 0).ToString(), index_stream_id},
            {true, rows->GetValue(1, 0).GetValue<std::uint64_t>(),
             rows->GetValue(2, 0).ToString(), rows->GetValue(3, 0).ToString(),
             rows->GetValue(4, 0).GetValue<bool>(), rows->GetValue(5, 0).GetValue<bool>()}};
}

CursorEpochManifest DuckdbPortfolioStore::stream_state(const RepositoryStreamKey& stream) const {
    validate_stream(stream); std::lock_guard<std::mutex> lock(store_mutex); return state_unlocked(stream);
}
...
ApplyResult DuckdbPortfolioStore::apply(const RepositoryStreamKey& stream,
    ...
    for (const auto& event : events) {
        auto found = receipt->Execute(event.event_id); ok(found, "read event receipt");
        auto row = found->Fetch();
        if (!row || row->size() != 1 ||
            row->GetValue(0, 0).ToString() != stream.repository_id ||
            row->GetValue(1, 0).ToString() != stream.index_stream_id ||
            row->GetValue(2, 0).GetValue<std::uint64_t>() != event.sequence ||
            row->GetValue(3, 0).ToString() != fingerprint(event)) { replay = false; break; }
    }
    if (replay) {
    ...
    auto insert_event = connection_->Prepare("INSERT INTO portfolio_events VALUES ($1,$2,$3,$4,$5)");
    ...
    for (const auto& event : events) {
        auto handoff = handoff_receipt->Execute(event.event_id, stream.index_stream_id,
                                                event.sequence);
        ...
        auto inserted = insert_event->Execute(event.event_id, stream.repository_id,
                                              stream.index_stream_id, event.sequence,
                                              fingerprint(event));
        ...
        for (const auto& mutation : event.mutations) {
            ...
            ok(result, "apply projection mutation"); ++changed;
    ...
    auto save = connection_->Prepare("INSERT INTO portfolio_streams VALUES ($1,$2,$3,$4,$5,$6,$7) "
    ...
    ok(save->Execute(stream.repository_id, stream.index_stream_id, state.cursor, state.epoch,
                     state.manifest, state.stale, state.removed), "save stream cursor");
    transaction.commit(); return {ApplyDisposition::Applied, state, changed};
}
...
StreamProjection DuckdbPortfolioStore::inspect_repository_stream(const RepositoryStreamKey& stream,
    ...
    std::lock_guard<std::mutex> lock(store_mutex);
    StreamProjection projection; projection.state = state_unlocked(stream);
    auto statement = connection_->Prepare("SELECT entity_kind,entity_key,digest FROM portfolio_entities "
                                          "WHERE repository_id=$1 AND index_stream_id=$2 "
                                          "ORDER BY entity_kind,entity_key LIMIT $3");
    auto result = statement->Execute(stream.repository_id, stream.index_stream_id, max_entities + 1);
...
ReplaceResult DuckdbPortfolioStore::replace_repository_stream(const RepositorySnapshot& snapshot,
    ...
    if (snapshot.cursor == 0 || snapshot.cursor < expected_cursor || snapshot.epoch.size() < 16 ||
        snapshot.epoch.size() > 128 || snapshot.manifest.size() < 16 || snapshot.manifest.size() > 128 ||
        snapshot.entities.size() > kMaxSnapshotEntities)
    ...
    for (const auto& entity : snapshot.entities) {
        ...
        if (entity.operation != ProjectionOperation::Upsert ||
            ...
            fail(PortfolioStoreErrorCode::InvalidInput, "invalid duplicate/deleted snapshot entity");
    }
    std::lock_guard<std::mutex> lock(store_mutex);
    Transaction transaction(*connection_);
    ...
    auto inspect = connection_->Prepare("SELECT entity_kind,entity_key,digest FROM portfolio_entities "
                                        "WHERE repository_id=$1 AND index_stream_id=$2");
    ...
    const CursorEpochManifest replacement = {true, snapshot.cursor, snapshot.epoch,
                                             snapshot.manifest, snapshot.stale, snapshot.removed};
    ...
    if (current.cursor != expected_cursor)
        fail(PortfolioStoreErrorCode::CursorConflict, "portfolio stream cursor conflict");
    auto erase = connection_->Prepare("DELETE FROM portfolio_entities WHERE repository_id=$1 AND index_stream_id=$2");
    ok(erase->Execute(snapshot.stream.repository_id, snapshot.stream.index_stream_id), "replace partition");
    ...
    for (const auto& [key, entity] : entities)
        ok(insert->Execute(snapshot.stream.repository_id, snapshot.stream.index_stream_id,
                           key.first, key.second, entity.digest.value_or("")), "insert snapshot entity");
    auto save = connection_->Prepare("INSERT INTO portfolio_streams VALUES ($1,$2,$3,$4,$5,$6,$7) "
                                     "ON CONFLICT(repository_id,index_stream_id) DO UPDATE SET "
                                     "cursor=excluded.cursor,epoch=excluded.epoch,"
                                     "manifest=excluded.manifest,stale=excluded.stale,removed=excluded.removed");
    ok(save->Execute(snapshot.stream.repository_id, snapshot.stream.index_stream_id, snapshot.cursor,
                     snapshot.epoch, snapshot.manifest, snapshot.stale, snapshot.removed), "replace stream state");
    transaction.commit();
    return {ApplyDisposition::Applied, replacement, entities.size()};
}

ApplyResult DuckdbPortfolioStore::reidentify_repository_stream(
    const RepositoryReidentification& value, std::uint64_t expected_cursor) {
    validate_reidentification(value, expected_cursor);
    std::lock_guard<std::mutex> lock(store_mutex);
    Transaction transaction(*connection_);
    auto receipt = connection_->Prepare(
        "SELECT fingerprint FROM portfolio_events WHERE event_id=$1 OR "
        "(index_stream_id=$2 AND sequence=$3)");
    auto receipt_result = receipt->Execute(value.event_id,
    ...
    auto orphan_detail = connection_->Prepare(
        "SELECT event_id FROM portfolio_reidentifications WHERE event_id=$1 OR "
        "(index_stream_id=$2 AND sequence=$3)");
    auto orphan_result = orphan_detail->Execute(value.event_id,
                                                value.previous_stream.index_stream_id,
                                                value.sequence);
    ...
    auto insert_global_receipt = connection_->Prepare(
        "INSERT INTO portfolio_events VALUES($1,$2,$3,$4,$5)");
    ...
    auto insert_receipt = connection_->Prepare(
        "INSERT INTO portfolio_reidentifications VALUES($1,$2,$3,$4,$5,$6)");
    ...
    auto transfer_entities = connection_->Prepare(
        "UPDATE portfolio_entities SET repository_id=$1 WHERE repository_id=$2 AND index_stream_id=$3");
    ...
    auto upsert_identity = connection_->Prepare(
        "INSERT INTO portfolio_entities VALUES($1,$2,'repository',$1,'') "
        "ON CONFLICT DO UPDATE SET digest=excluded.digest");
