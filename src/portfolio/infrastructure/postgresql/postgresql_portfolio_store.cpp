...
Result execute(PGconn* connection, const std::string& sql,
    ...
    return checked(PQexecParams(connection, sql.c_str(), static_cast<int>(values.size()), nullptr,
                                values.data(), nullptr, nullptr, 0), operation, expected);
}
...
bool uuid(const std::string& value) {
    ...
    for (std::size_t index = 0; index < value.size(); ++index) {
        ...
        } else if (!std::isxdigit(static_cast<unsigned char>(value[index]))) return false;
    }
    return true;
}
...
class Fingerprint {
public:
    explicit Fingerprint(const char* domain) { blake3_hasher_init(&hasher_); add(domain); }
...
std::string fingerprint(const ProjectionEvent& event) {
    ...
    hash.add(event.stream.repository_id); hash.add(event.stream.index_stream_id);
    hash.add(std::to_string(event.sequence)); hash.add(event.event_id); hash.add(event.epoch);
    hash.add(event.manifest.value_or(""));
    for (const auto& mutation : event.mutations) {
        hash.add(mutation.entity_kind); hash.add(mutation.entity_key);
...
std::string fingerprint(const RepositoryReidentification& value) {
    ...
    hash.add(value.previous_stream.repository_id); hash.add(value.previous_stream.index_stream_id);
    hash.add(value.current_stream.repository_id); hash.add(value.current_stream.index_stream_id);
    hash.add(std::to_string(value.sequence)); hash.add(value.event_id); hash.add(value.epoch);
    hash.add(value.manifest.value_or("")); hash.add(value.old_binding_id);
    hash.add(value.new_binding_id); hash.add(value.approval_reference); hash.add(value.reason);
    return hash.finish();
}
...
PostgresqlPortfolioStore::PostgresqlPortfolioStore(std::string connection_string,
                                                   std::string schema,
                                                   bool drop_schema_on_destroy)
    : schema_(std::move(schema)),
      drop_schema_on_destroy_(drop_schema_on_destroy) {
    if (schema_.empty() || schema_.size() > 63 || schema_[0] < 'a' || schema_[0] > 'z' ||
        !std::all_of(schema_.begin(), schema_.end(), [](unsigned char character) {
            return std::islower(character) || std::isdigit(character) || character == '_';
        }) || schema_ == "public" || schema_ == "information_schema" ||
        schema_.rfind("pg_", 0) == 0)
...
PostgresqlPortfolioStore::~PostgresqlPortfolioStore() {
    ...
    if (drop_schema_on_destroy_ && schema_created_by_this_instance_) {
        auto* result = PQexec(connection_, ("DROP SCHEMA IF EXISTS " + schema_ + " CASCADE").c_str());
        if (result) PQclear(result);
    }
    PQfinish(connection_);
}

std::string PostgresqlPortfolioStore::table(const char* name) const { return schema_ + "." + name; }

void PostgresqlPortfolioStore::migrate() {
    std::lock_guard<std::mutex> lock(mutex_);
    Transaction transaction(connection_);
    lock_stream(connection_, "schema:" + schema_);
    auto schema_exists = execute(connection_,
        "SELECT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname=$1)", {schema_},
        "inspect schema ownership");
    ...
    command(connection_, "CREATE TABLE IF NOT EXISTS " + table("portfolio_schema_migrations") +
        "(version integer PRIMARY KEY,checksum text NOT NULL)", "create migration ledger");
    auto migrations = execute(connection_, "SELECT version,checksum FROM " +
        table("portfolio_schema_migrations") + " ORDER BY version", {}, "read migration ledger");
    bool has_v1 = false;
    bool has_v2 = false;
    for (int row = 0; row < PQntuples(migrations.get()); ++row) {
        ...
        if (version == 1 && checksum == "axon/portfolio-postgresql/v1") has_v1 = true;
        else if (version == 2 && checksum == "axon/portfolio-postgresql-outbox/v2") has_v2 = true;
        else fail(PortfolioStoreErrorCode::IdempotencyConflict, "migration checksum mismatch");
    }
    command(connection_, "CREATE TABLE IF NOT EXISTS " + table("portfolio_streams") +
        "(repository_id text NOT NULL,index_stream_id text NOT NULL UNIQUE,cursor numeric(20,0) "
        "NOT NULL,epoch text NOT NULL,manifest text NOT NULL,stale boolean NOT NULL,removed boolean "
        "NOT NULL,PRIMARY KEY(repository_id,index_stream_id))", "create streams");
    command(connection_, "CREATE TABLE IF NOT EXISTS " + table("portfolio_entities") +
        "(repository_id text NOT NULL,index_stream_id text NOT NULL,entity_kind text NOT NULL,"
        "entity_key text NOT NULL,digest text,PRIMARY KEY(repository_id,index_stream_id,entity_kind,"
        "entity_key))", "create entities");
    command(connection_, "CREATE TABLE IF NOT EXISTS " + table("portfolio_events") +
        "(event_id text PRIMARY KEY,repository_id text NOT NULL,index_stream_id text NOT NULL,"
        "sequence numeric(20,0) NOT NULL,fingerprint text NOT NULL,UNIQUE(index_stream_id,sequence))",
        "create events");
    if (!has_v1)
        command(connection_, "INSERT INTO " + table("portfolio_schema_migrations") +
            " VALUES(1,'axon/portfolio-postgresql/v1')", "record core migration");
    command(connection_, "CREATE TABLE IF NOT EXISTS " + table("portfolio_reidentifications") +
        "(event_id text PRIMARY KEY,index_stream_id text NOT NULL,sequence numeric(20,0) NOT NULL,"
        "previous_repository_id text NOT NULL,current_repository_id text NOT NULL,fingerprint text "
        "NOT NULL,UNIQUE(index_stream_id,sequence))", "create reidentifications");
    command(connection_, "CREATE TABLE IF NOT EXISTS " + table("portfolio_outbox") +
        "(event_id text PRIMARY KEY,event_kind text NOT NULL,repository_id text NOT NULL,"
        "index_stream_id text NOT NULL,sequence numeric(20,0) NOT NULL,published boolean NOT NULL "
        "DEFAULT false,created_at timestamptz NOT NULL DEFAULT clock_timestamp())", "create outbox");
    if (!has_v2)
        command(connection_, "INSERT INTO " + table("portfolio_schema_migrations") +
            " VALUES(2,'axon/portfolio-postgresql-outbox/v2')", "record outbox migration");
    transaction.commit();
}
...
ProviderCapabilities PostgresqlPortfolioStore::capabilities() const {
    return {ProviderRole::PortfolioStore,
            ...
            kMaxEvents, kMaxEvents, kMaxMutations, kMaxSnapshot};
}
...
std::string PostgresqlPortfolioStore::schema_version() const { return "axon/portfolio-store/v1"; }
std::string PostgresqlPortfolioStore::protocol_version() const {
    return "axon/portfolio-provider/v1";
}

CursorEpochManifest PostgresqlPortfolioStore::state_unlocked(
    const RepositoryStreamKey& stream, bool lock_row) const {
    auto result = execute(connection_, "SELECT cursor,epoch,manifest,stale,removed FROM " +
        table("portfolio_streams") + " WHERE repository_id=$1 AND index_stream_id=$2" +
        (lock_row ? " FOR UPDATE" : ""), {stream.repository_id, stream.index_stream_id},
        "read stream");
    if (PQntuples(result.get()) == 0) return {};
    return {true, number(result.get(), 0, 0), PQgetvalue(result.get(), 0, 1),
            PQgetvalue(result.get(), 0, 2), boolean(result.get(), 0, 3),
...
PostgresqlPortfolioStore::physical_state_unlocked(const std::string& index_stream_id,
                                                   bool lock_row) const {
    auto result = execute(connection_, "SELECT repository_id,cursor,epoch,manifest,stale,removed "
        "FROM " + table("portfolio_streams") + " WHERE index_stream_id=$1" +
        (lock_row ? " FOR UPDATE" : ""), {index_stream_id}, "read physical stream");
    if (PQntuples(result.get()) == 0) return {};
    if (PQntuples(result.get()) != 1)
        fail(PortfolioStoreErrorCode::IdempotencyConflict,
             "physical stream has multiple bindings");
...
CursorEpochManifest PostgresqlPortfolioStore::stream_state(
    const RepositoryStreamKey& stream) const {
...
ApplyResult PostgresqlPortfolioStore::apply(const RepositoryStreamKey& stream,
    ...
    bool replay = true;
    for (const auto& event : events) {
        auto receipt = execute(connection_, "SELECT repository_id,index_stream_id,sequence,"
            "fingerprint FROM " + table("portfolio_events") + " WHERE event_id=$1",
    ...
    if (replay) {
        ...
        if (!state.exists)
            fail(PortfolioStoreErrorCode::IdempotencyConflict,
                 "receipt has no physical stream");
        transaction.commit();
    ...
    try {
        for (const auto& event : events) {
            execute(connection_, "INSERT INTO " + table("portfolio_events") +
                " VALUES($1,$2,$3,$4,$5)", {event.event_id, stream.repository_id,
                stream.index_stream_id, std::to_string(event.sequence), fingerprint(event)},
                "insert receipt", PGRES_COMMAND_OK);
            for (const auto& mutation : event.mutations) {
                if (mutation.operation == ProjectionOperation::Delete)
                    execute(connection_, "DELETE FROM " + table("portfolio_entities") +
                        " WHERE repository_id=$1 AND index_stream_id=$2 AND entity_kind=$3 AND "
                        "entity_key=$4", {stream.repository_id, stream.index_stream_id,
                        mutation.entity_kind, mutation.entity_key}, "delete entity", PGRES_COMMAND_OK);
                else
                    execute(connection_, "INSERT INTO " + table("portfolio_entities") +
            ...
            state = {true, event.sequence, event.epoch,
                     event.manifest.value_or(state.manifest), false, state.removed};
            execute(connection_, "INSERT INTO " + table("portfolio_outbox") +
                "(event_id,event_kind,repository_id,index_stream_id,sequence) VALUES($1,'projection',"
                "$2,$3,$4)", {event.event_id, stream.repository_id, stream.index_stream_id,
                std::to_string(event.sequence)}, "insert outbox", PGRES_COMMAND_OK);
        }
        execute(connection_, "INSERT INTO " + table("portfolio_streams") +
            " VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(repository_id,index_stream_id) DO UPDATE SET "
            "cursor=excluded.cursor,epoch=excluded.epoch,manifest=excluded.manifest,"
            "stale=excluded.stale,removed=excluded.removed", {stream.repository_id,
            stream.index_stream_id, std::to_string(state.cursor), state.epoch, state.manifest,
            state.stale ? "true" : "false", state.removed ? "true" : "false"},
            "save stream", PGRES_COMMAND_OK);
    } catch (const SqlError& error) { map_conflict(error); }
    transaction.commit();
    return {ApplyDisposition::Applied, state, changed};
}

StreamProjection PostgresqlPortfolioStore::inspect_repository_stream(
    const RepositoryStreamKey& stream, std::size_t max_entities) const {
    validate_stream(stream);
    ...
    auto rows = execute(connection_, "SELECT entity_kind,entity_key,digest FROM " +
        table("portfolio_entities") + " WHERE repository_id=$1 AND index_stream_id=$2 ORDER BY "
        "entity_kind,entity_key LIMIT $3", {stream.repository_id, stream.index_stream_id,
        std::to_string(max_entities + 1)}, "inspect entities");
    for (int row = 0; row < PQntuples(rows.get()); ++row) {
        ...
        projection.entities.push_back({PQgetvalue(rows.get(), row, 0),
            PQgetvalue(rows.get(), row, 1), ProjectionOperation::Upsert,
            digest.empty() ? std::nullopt : std::optional<std::string>(digest)});
    }
    return projection;
}

ReplaceResult PostgresqlPortfolioStore::replace_repository_stream(
    const RepositorySnapshot& snapshot, std::uint64_t expected_cursor) {
    ...
    if (!current.exists && physical_state_unlocked(snapshot.stream.index_stream_id, true).second.exists)
        fail(PortfolioStoreErrorCode::IdempotencyConflict,
             "physical stream is bound to another repository");
    if (current.cursor != expected_cursor)
        fail(PortfolioStoreErrorCode::CursorConflict, "portfolio stream cursor conflict");
    auto existing = execute(connection_, "SELECT entity_kind,entity_key,digest FROM " +
        table("portfolio_entities") + " WHERE repository_id=$1 AND index_stream_id=$2",
    ...
    const CursorEpochManifest replacement{true, snapshot.cursor, snapshot.epoch, snapshot.manifest,
                                          snapshot.stale, snapshot.removed};
    if (current == replacement && old_entities == entities) {
        transaction.commit();
        return {ApplyDisposition::Duplicate, current, 0};
    }
    execute(connection_, "DELETE FROM " + table("portfolio_entities") +
        " WHERE repository_id=$1 AND index_stream_id=$2", {snapshot.stream.repository_id,
        snapshot.stream.index_stream_id}, "clear partition", PGRES_COMMAND_OK);
    for (const auto& [key, entity] : entities)
        execute(connection_, "INSERT INTO " + table("portfolio_entities") +
            " VALUES($1,$2,$3,$4,$5)", {snapshot.stream.repository_id,
            snapshot.stream.index_stream_id, key.first, key.second, entity.digest.value_or("")},
            "insert snapshot entity", PGRES_COMMAND_OK);
    execute(connection_, "INSERT INTO " + table("portfolio_streams") +
        " VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT(repository_id,index_stream_id) DO UPDATE SET "
        "cursor=excluded.cursor,epoch=excluded.epoch,manifest=excluded.manifest,"
        "stale=excluded.stale,removed=excluded.removed", {snapshot.stream.repository_id,
        snapshot.stream.index_stream_id, std::to_string(snapshot.cursor), snapshot.epoch,
        snapshot.manifest, snapshot.stale ? "true" : "false",
        snapshot.removed ? "true" : "false"}, "replace stream", PGRES_COMMAND_OK);
    execute(connection_, "INSERT INTO " + table("portfolio_outbox") +
        ...
        {snapshot.stream.repository_id, snapshot.stream.index_stream_id,
         std::to_string(snapshot.cursor)}, "replace outbox",
        PGRES_COMMAND_OK);
    transaction.commit();
    return {ApplyDisposition::Applied, replacement, entities.size()};
}

ApplyResult PostgresqlPortfolioStore::reidentify_repository_stream(
    ...
    auto receipt = execute(connection_, "SELECT fingerprint FROM " + table("portfolio_events") +
        " WHERE event_id=$1 OR (index_stream_id=$2 AND sequence=$3)", {value.event_id,
        value.previous_stream.index_stream_id, std::to_string(value.sequence)}, "read handoff receipt");
    if (PQntuples(receipt.get()) != 0) {
        if (PQntuples(receipt.get()) != 1 ||
            std::string(PQgetvalue(receipt.get(), 0, 0)) != fingerprint(value))
            fail(PortfolioStoreErrorCode::IdempotencyConflict,
                 "reidentification receipt conflict");
        auto detail = execute(connection_, "SELECT fingerprint FROM " +
            table("portfolio_reidentifications") + " WHERE event_id=$1", {value.event_id},
            "read handoff detail");
        const auto current = physical_state_unlocked(value.current_stream.index_stream_id, true).second;
        if (PQntuples(detail.get()) != 1 ||
            std::string(PQgetvalue(detail.get(), 0, 0)) != fingerprint(value) ||
            !current.exists || current.cursor < value.sequence)
            fail(PortfolioStoreErrorCode::IdempotencyConflict,
    ...
    try {
        execute(connection_, "INSERT INTO " + table("portfolio_events") +
            " VALUES($1,$2,$3,$4,$5)", {value.event_id, value.previous_stream.repository_id,
            value.previous_stream.index_stream_id, std::to_string(value.sequence),
            fingerprint(value)}, "insert handoff receipt", PGRES_COMMAND_OK);
        execute(connection_, "INSERT INTO " + table("portfolio_reidentifications") +
            " VALUES($1,$2,$3,$4,$5,$6)", {value.event_id,
            value.previous_stream.index_stream_id, std::to_string(value.sequence),
            value.previous_stream.repository_id, value.current_stream.repository_id,
            fingerprint(value)}, "insert handoff detail", PGRES_COMMAND_OK);
        execute(connection_, "UPDATE " + table("portfolio_entities") +
            ...
            {value.current_stream.repository_id, value.previous_stream.repository_id,
             value.previous_stream.index_stream_id}, "transfer entities", PGRES_COMMAND_OK);
        execute(connection_, "DELETE FROM " + table("portfolio_entities") +
            " WHERE repository_id=$1 AND index_stream_id=$2 AND entity_kind='repository' AND "
            "entity_key=$3", {value.current_stream.repository_id,
            value.current_stream.index_stream_id, value.previous_stream.repository_id},
        ...
        execute(connection_, "UPDATE " + table("portfolio_streams") +
            " SET repository_id=$1,cursor=$2,epoch=$3,manifest=$4,stale=false,removed=false "
            "WHERE repository_id=$5 AND index_stream_id=$6", {value.current_stream.repository_id,
            std::to_string(value.sequence), value.epoch, manifest,
            value.previous_stream.repository_id, value.previous_stream.index_stream_id},
            "transfer stream", PGRES_COMMAND_OK);
        execute(connection_, "INSERT INTO " + table("portfolio_outbox") +
            "(event_id,event_kind,repository_id,index_stream_id,sequence) VALUES($1,'reidentify',"
            "$2,$3,$4)", {value.event_id, value.current_stream.repository_id,
        ...
        return {ApplyDisposition::Applied,
                {true, value.sequence, value.epoch, manifest, false, false}, 2};
    } catch (const SqlError& error) { map_conflict(error); }
...
MaintenanceResult PostgresqlPortfolioStore::maintenance(MaintenanceKind kind) {
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    auto invalid = execute(connection_, "SELECT count(*) FROM " + table("portfolio_streams") +
        " WHERE cursor=0", {}, "validate cursors");
    if (number(invalid.get(), 0, 0) != 0)
        fail(PortfolioStoreErrorCode::IdempotencyConflict, "invalid zero cursor");
    auto orphan = execute(connection_, "SELECT count(*) FROM " +
        table("portfolio_reidentifications") + " r LEFT JOIN " + table("portfolio_events") +
...
std::size_t PostgresqlPortfolioStore::pending_outbox_count() const {
    std::lock_guard<std::mutex> lock(mutex_);
    auto count = execute(connection_, "SELECT count(*) FROM " + table("portfolio_outbox") +
        " WHERE published=false", {}, "count outbox");
...
std::unique_ptr<PortfolioStore> make_postgresql_conformance_store() {
    ...
    return std::make_unique<PostgresqlPortfolioStore>(dsn,
        "axon_test_conformance_" + std::to_string(stamp) + "_" +
        std::to_string(ordinal.fetch_add(1)), true);
}
