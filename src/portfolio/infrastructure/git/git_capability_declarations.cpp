...
bool is_object_id(const std::string& value) {
    return (value.size() == 40U || value.size() == 64U) &&
        std::all_of(value.begin(), value.end(), [](const unsigned char c) { return std::isxdigit(c); });
}
...
void require_regular_git_blob(const std::filesystem::path& root, const std::string& sha,
    ...
    const auto listing = git::run(root, {"ls-tree", "-z", sha, "--", relative.generic_string()}, 4096U);
    ...
    if (listing.exit_code != 0 || listing.output_truncated ||
        (listing.stdout_text.rfind(expected_prefix, 0) != 0 && listing.stdout_text.rfind(executable_prefix, 0) != 0)) {
        throw std::invalid_argument("declaration fragment is not a regular Git blob");
    }
}
} // namespace

DeclarationImportResult CapabilityDeclarationImporter::import(const std::filesystem::path& root,
                                                               const std::filesystem::path& relative) const {
    const auto canonical = std::filesystem::canonical(root);
    if (!std::filesystem::is_directory(canonical)) throw std::invalid_argument("declaration root is unavailable");
    ...
    const auto content = git::run(canonical,
    ...
    if (json.value("schema_version", "") != "axon/capability-graph/v1" || !json.contains("capabilities") ||
        !json["capabilities"].is_array()) throw std::invalid_argument("unsupported declaration schema");
    DeclarationImportResult out;
    for (const auto& item : json["capabilities"]) {
        if (!item.is_object() || !item.contains("id") || !item.contains("name") || !item["id"].is_string() ||
            !item["name"].is_string()) throw std::invalid_argument("invalid capability declaration");
        ...
        declaration.source_repository = canonical.filename();
        declaration.source_commit = sha;
        declaration.source_path = safe_relative.generic_string();
        if (declaration.id.empty() || declaration.normalized_name.empty() || declaration.contracts.size() > 2000U) {
