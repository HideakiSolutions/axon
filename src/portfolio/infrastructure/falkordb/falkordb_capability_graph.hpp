...
public:
    ...
    void replace_repository(const RepositoryStreamKey& stream, const std::string& generation,
                            const std::vector<CapabilitySignature>& signatures) override;
    GraphTraversal traverse(const RepositoryStreamKey& stream, const std::string& signature_id,
                            std::size_t max_depth, std::size_t max_nodes) const override;
