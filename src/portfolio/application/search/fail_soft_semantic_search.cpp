...
FailSoftSemanticSearch::FailSoftSemanticSearch(SemanticCapabilityStore& primary, SemanticCapabilityStore& fallback)
    : primary_(primary), fallback_(fallback) {}

SemanticSearchResult FailSoftSemanticSearch::search(const std::vector<float>& vector, const SemanticIdentity& identity,
    ...
    try {
        ...
        return {fallback_.search(vector, identity, limit), SemanticSearchMode::fallback, error.what()};
    }
}
...
void FailSoftSemanticSearch::upsert(const SemanticRecord& record) {
    ...
    try { primary_.upsert(record); }
    catch (const std::exception&) { primary_dirty_.store(true); }
}
...
void FailSoftSemanticSearch::erase(const std::string& signature_id, const std::string& generation) {
    ...
    try { primary_.erase(signature_id, generation); }
    catch (const std::exception&) { primary_dirty_.store(true); }
}

void FailSoftSemanticSearch::mark_primary_reconciled() { primary_dirty_.store(false); }
bool FailSoftSemanticSearch::primary_dirty() const { return primary_dirty_.load(); }
} // namespace axon::portfolio
