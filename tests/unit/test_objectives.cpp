...
TEST_F(ObjTest, ObservationTagsAreUniqueAndExistingRowsRemainValid) {
    db->conn().Query(
        "INSERT INTO observations (id, content, created_at) "
        "VALUES (nextval('seq_id'), 'legacy-row', now())");
    auto id = db->conn().Query("SELECT id FROM observations WHERE content = 'legacy-row'")
                  ->GetValue<int64_t>(0, 0);

    auto first = db->conn().Query("INSERT INTO observation_tags VALUES (" +
                                  std::to_string(id) + ", 'migration')");
    ASSERT_FALSE(first->HasError()) << first->GetError();
    auto duplicate = db->conn().Query("INSERT INTO observation_tags VALUES (" +
                                      std::to_string(id) + ", 'migration')");
