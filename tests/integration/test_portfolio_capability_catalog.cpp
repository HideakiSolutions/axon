...
struct SourceFile { std::string path, symbol, route, hash; };
...
void create_source(const fs::path& source, const char* repository_id, const char* stream_id,
    ...
    ASSERT_FALSE(c.Query("CREATE TABLE capability_contexts(file_id BIGINT,bounded_context VARCHAR)")->HasError());
    ASSERT_FALSE(c.Query("CREATE TABLE capability_ast_fingerprints(file_id BIGINT,value VARCHAR)")->HasError());
    ...
    for (std::size_t i=0;i<files.size();++i) {
        ...
        ASSERT_FALSE(c.Query("INSERT INTO files VALUES("+id+",'"+file.path+"','cpp','"+file.hash+"')")->HasError());
        ASSERT_FALSE(c.Query("INSERT INTO symbols VALUES("+id+",'"+file.symbol+"')")->HasError());
        ASSERT_FALSE(c.Query("INSERT INTO routes VALUES('"+file.path+"','POST','"+file.route+"')")->HasError());
        ...
        if (file.path.rfind("src/",0)==0 && slash!=std::string::npos) {
            ...
            ASSERT_FALSE(c.Query("INSERT INTO capability_contexts VALUES("+id+",'"+context+"')")->HasError());
        }
        ASSERT_FALSE(c.Query("INSERT INTO capability_ast_fingerprints VALUES("+id+",'"+file.hash+"')")->HasError());
        if (file.path.find("payment_authorize") != std::string::npos) {
            ASSERT_FALSE(c.Query("INSERT INTO external_dependencies VALUES("+id+",'@axon/shared-contracts','imports')")->HasError());
        }
...
TEST(PortfolioCapabilityCatalog, SyncsRealReadOnlySourcesAndSearchesMetadata) {
    ...
    create_source(source,kFirstRepository,kFirstStream,{{"src/billing/payment_authorize.cpp","authorize","/pay","0123456789abcdef"},{"src/billing/invoice_create.cpp","create_invoice","/invoice","1111111111111111"},{"src/billing/order_policy.cpp","apply_policy","/order","2222222222222222"},{"src/billing/status_check.cpp","status","/status","3333333333333333"},{"src/catalog/search.cpp","query","/search","fedcba9876543210"}});
    create_source(second_source,kSecondRepository,kSecondStream,{{"src/billing/payment_authorize.cpp","authorize","/pay","0123456789abcdef"},{"src/billing/invoice_create.cpp","create_invoice","/invoice","9999999999999999"},{"src/billing/workflow_policy.cpp","apply_policy","/workflow","2222222222222222"}});
    create_source(third_source,kThirdRepository,kThirdStream,{{"src/billing/payment_authorize.cpp","authorize","/pay","0123456789abcdef"},{"src/identity/status_check.cpp","status","/status","3333333333333333"}});
    std::ofstream(base/"repo/capabilities.json") << R"({"schema_version":"axon/capability-graph/v1","capabilities":[{"id":"payments.authorize","name":"payment authorize","bounded_context":"billing","contracts":["AuthorizeRequest"]},{"id":"platform.unimplemented","name":"unimplemented capability","bounded_context":"platform","contracts":[]}]})";
    ...
    EXPECT_EQ(search->internal_dependencies,std::vector<std::string>({"src/billing/payment_authorize.cpp"}));
    EXPECT_EQ(std::count_if(signatures.begin(),signatures.end(),[](const auto& signature) {
        return signature.path && *signature.path=="src/billing/payment_authorize.cpp" &&
    ...
    const auto find=[&](const char* repository,const char* path) {
        const auto it=std::find_if(signatures.begin(),signatures.end(),[&](const auto& signature) {
            return signature.stream.repository_id==repository && signature.path && *signature.path==path;
        });
        EXPECT_NE(it,signatures.end()); return *it;
    };
    ...
    EXPECT_EQ(axon::portfolio::CapabilityCandidateGenerator().generate({payment_one,payment_two}).front().classification,axon::portfolio::CapabilityClassification::exact_duplicate);
    ...
    EXPECT_EQ(axon::portfolio::CapabilityCandidateGenerator().generate({invoice_one,invoice_two}).front().classification,axon::portfolio::CapabilityClassification::convergent_capability);
    ...
    EXPECT_EQ(axon::portfolio::CapabilityCandidateGenerator().generate({order,workflow}).front().classification,axon::portfolio::CapabilityClassification::local_specialization);
    ...
    EXPECT_EQ(axon::portfolio::CapabilityCandidateGenerator().generate({status_one,status_three}).front().classification,axon::portfolio::CapabilityClassification::semantic_coincidence);

    const auto comparison=catalog.drift(base/"repo","capabilities.json");
    EXPECT_FALSE(comparison.matches.empty());
    EXPECT_NE(std::find_if(comparison.drift.begin(),comparison.drift.end(),[](const auto& drift) { return drift.kind==axon::portfolio::CapabilityDriftKind::declaration_without_observed; }),comparison.drift.end());
    EXPECT_NE(std::find_if(comparison.drift.begin(),comparison.drift.end(),[](const auto& drift) { return drift.kind==axon::portfolio::CapabilityDriftKind::observed_without_declaration; }),comparison.drift.end());
}
...
TEST(PortfolioCapabilityCatalog, ProjectsIndexerProducedEvidenceFromThreeRepositories) {
    ...
    fs::remove_all(base);
    struct Repo { const char* id; const char* stream; fs::path root; };
    const std::array<Repo,3> repos{{{kFirstRepository,kFirstStream,base/"one"},{kSecondRepository,kSecondStream,base/"two"},{kThirdRepository,kThirdStream,base/"three"}}};
    ...
    for(std::size_t i=0;i<repos.size();++i) {
        ...
        std::ofstream(repo.root/"repository-contract.yaml")<<"schema_version: repository-contract/v1\nrepository_id: "<<repo.id<<"\n";
        std::ofstream(repo.root/"src/billing/payment_authorize.ts")<<"export function authorize() { return true; }\n";
        ...
        ASSERT_FALSE(evidence->HasError()); ASSERT_EQ(evidence->GetValue<int64_t>(0,0),1);
    }
    fs::create_directories(base/"registry");
    std::ofstream registry(base/"registry/registry.json");
    registry<<"{\"schema_version\":\"axon-registry/v2\",\"repos\":[";
    for(std::size_t i=0;i<repos.size();++i) { if(i) registry<<','; const auto& repo=repos[i]; registry<<"{\"name\":\"r"<<i<<"\",\"root\":\""<<repo.root.string()<<"\",\"db_path\":\""<<(repo.root/".axon/index.duckdb").string()<<"\",\"repository_id\":\""<<repo.id<<"\",\"index_stream_id\":\""<<streams[i]<<"\",\"default_for_profiles\":[\"local\"]}"; }
    registry<<"],\"storage_profiles\":{\"local\":{\"role\":\"portfolio_local\",\"transport\":\"local\",\"default\":true,\"portfolio_store\":{\"provider\":\"duckdb\",\"path\":\"x\"}}}}";
    registry.close(); setenv("AXON_REGISTRY_DIR",(base/"registry").c_str(),1);
    {
    ...
    ASSERT_TRUE(catalog.list(kFirstRepository,10).front().ast_fingerprints.empty());
    {
        ...
        EXPECT_EQ(db.conn().Query("SELECT COUNT(*) FROM capability_ast_fingerprints")->GetValue<int64_t>(0,0),1);
    }
    const auto refreshed=catalog.sync();
    ASSERT_FALSE(refreshed.degraded);
    const auto refreshed_first=std::find_if(refreshed.repositories.begin(),refreshed.repositories.end(),[](const auto& item) { return item.repository_id==kFirstRepository; });
    ...
    EXPECT_EQ(candidates.front().classification,axon::portfolio::CapabilityClassification::exact_duplicate);
}
}
