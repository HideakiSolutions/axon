...
namespace {
std::string dsn() { const char* value = std::getenv("AXON_PORTFOLIO_SEMANTIC_DSN"); return value ? value : ""; }
#ifdef AXON_HAVE_QDRANT
struct QdrantCleanup { axon::portfolio::QdrantSemanticStore& store; std::string id, generation; ~QdrantCleanup(){ try { store.erase(id,generation); } catch (...) {} } };
#endif
struct PgCleanup {
    axon::portfolio::PgvectorSemanticStore& store;
    std::vector<std::pair<std::string, std::string>> records;
    ~PgCleanup() { for (const auto& [id, generation] : records) try { store.erase(id, generation); } catch (...) {} }
};
...
public:
    ...
    bool contains = false;
    void upsert(const axon::portfolio::SemanticRecord&) override { if (unavailable) throw std::runtime_error("unavailable"); contains = true; }
    void erase(const std::string&, const std::string&) override { if (unavailable) throw std::runtime_error("unavailable"); contains = false; }
    std::vector<axon::portfolio::SemanticHit> search(const std::vector<float>&, const axon::portfolio::SemanticIdentity&, std::size_t) const override {
        if (unavailable) throw std::runtime_error("unavailable"); return contains ? hits : std::vector<axon::portfolio::SemanticHit>{};
    }
};
TEST(PgvectorSemanticStore, UpsertsFiltersAndDeletesByGeneration) {
    ...
    PgCleanup cleanup{store, {{"aaaaaaaaaaaaaaaa", "generation-a"}, {"bbbbbbbbbbbbbbbb", "generation-a"},
                              {"cccccccccccccccc", "generation-a"}, {"cccccccccccccccc", "generation-b"},
                              {"cccccccccccccccc", "generation-c"}}};
    const axon::portfolio::SemanticIdentity identity{"test-model",3,"cosine","generation-a"};
    store.upsert({"aaaaaaaaaaaaaaaa", "7359f9cf-c2e0-4a61-ab7b-a5fd0918cbbb", "epoch-aaaaaaaaaaaa", identity,{1,0,0}});
    store.upsert({"bbbbbbbbbbbbbbbb", "7359f9cf-c2e0-4a61-ab7b-a5fd0918cbbb", "epoch-bbbbbbbbbbbb", identity,{0,1,0}});
    const auto hits=store.search({1,0,0},identity,2);
    ASSERT_EQ(hits.size(),2u); EXPECT_EQ(hits.front().signature_id,"aaaaaaaaaaaaaaaa");
    ...
    store.upsert({"cccccccccccccccc", "7359f9cf-c2e0-4a61-ab7b-a5fd0918cbbb", "epoch-cccccccccccc", identity,{1,0,0}});
    store.upsert({"cccccccccccccccc", "7359f9cf-c2e0-4a61-ab7b-a5fd0918cbbb", "epoch-dddddddddddd", next,{0,1,0}});
    EXPECT_EQ(store.search({1,0,0},identity,2).size(),1u);
    ...
    store.upsert({"cccccccccccccccc", "7359f9cf-c2e0-4a61-ab7b-a5fd0918cbbb", "epoch-eeeeeeeeeeee", next_model,{0,0,1}});
...
TEST(FailSoftSemanticSearch, KeepsFallbackWarmAcrossOutageAndDelete) {
    ...
    axon::portfolio::FailSoftSemanticSearch search(primary, fallback);
    const axon::portfolio::SemanticIdentity identity{"test-model", 3, "cosine", "generation-a"};
    const axon::portfolio::SemanticRecord record{"signature", "7359f9cf-c2e0-4a61-ab7b-a5fd0918cbbb", "epoch", identity, {1, 0, 0}};
...
TEST(QdrantSemanticStore, UpsertsSearchesAndDeletesByGeneration) {
    ...
    axon::portfolio::QdrantSemanticStore store("http://127.0.0.1:6333",key,"axon-portfolio-capabilities-v1",768);
    const axon::portfolio::SemanticIdentity identity{"test-model",768,"cosine","generation-qdrant"};
    std::vector<float> vector(768); vector[0]=1;
    store.upsert({"11111111111111111111111111111111", "7359f9cf-c2e0-4a61-ab7b-a5fd0918cbbb", "epoch-111111111111", identity,vector});
    QdrantCleanup cleanup{store,"11111111111111111111111111111111","generation-qdrant"};
    std::vector<float> updated(768); updated[1]=1;
    store.upsert({"11111111111111111111111111111111", "7359f9cf-c2e0-4a61-ab7b-a5fd0918cbbb", "epoch-222222222222", identity,updated});
    const auto hits=store.search(vector,identity,2);
    ASSERT_EQ(hits.size(), 1u); EXPECT_EQ(hits.front().signature_id,"11111111111111111111111111111111");
