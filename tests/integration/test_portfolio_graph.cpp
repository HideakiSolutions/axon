...
axon::portfolio::CapabilitySignature signature(const std::string& repo, const std::string& stream,
    ...
    value.stream = {repo, stream};
    value.index_epoch = "epoch_1";
    return value;
}
TEST(FalkorDbCapabilityGraph, ReplaceIsolatedAndBounded) {
    ...
    b.call_graph_neighborhood.push_back(
        {"incoming", "calls:typed/path", "dddddddddddddddd", 2, "0123456789abcdef"});
    graph.replace_repository(first, "generation_1", {a, b, c});
    graph.replace_repository(
        second, "generation_1",
        {signature(second.repository_id, second.index_stream_id, "cccccccccccccccc")});
    graph.replace_repository(
        sibling, "generation_1",
        {signature(sibling.repository_id, sibling.index_stream_id, "bbbbbbbbbbbbbbbb")});
    const auto first_nodes = graph.traverse(first, a.signature_id, 2, 10);
    EXPECT_FALSE(first_nodes.truncated);
    EXPECT_GE(first_nodes.capability_ids.size(), 1u);
    EXPECT_EQ(std::find(first_nodes.capability_ids.begin(), first_nodes.capability_ids.end(),
                        first.repository_id + ":" + first.index_stream_id + ":" + c.signature_id),
              first_nodes.capability_ids.end());
    ...
    EXPECT_THROW(graph.replace_repository(first, "generation_4", {b_without_edge, invalid}),
                 std::invalid_argument);
    auto invalid_neighbor = b_without_edge;
    invalid_neighbor.call_graph_neighborhood.push_back(
        {"sideways", "calls", "aaaaaaaaaaaaaaaa", 1, std::nullopt});
    EXPECT_THROW(graph.replace_repository(first, "generation_4", {invalid_neighbor}),
                 std::invalid_argument);
