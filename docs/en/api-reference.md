...
| `axon watch [path] [--interval-ms=N] [--debounce-ms=N] [--backend=auto\|native\|poll]` | Watch for external edits and incrementally reindex. `auto` (default) uses the native backend — inotify on Linux, FSEvents on macOS — and falls back to portable polling with a warning when native init fails; `native` fails hard instead of falling back (diagnostics); `poll` forces the portable watcher (Windows default). Kernel event-queue overflow triggers a full rescan; `AXON_WATCH_FORCE_POLL=1` forces polling from the environment |
