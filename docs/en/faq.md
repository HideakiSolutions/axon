...
No. All 15 MCP tools work without it. The embedding model only enables `search_memory` and the semantic-query path of `get_context_capsule`. Without it, `get_context_capsule` falls back to graph-based pivot selection.
